...
class RewrapCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'envserver:rewrap';
    ...
    public function handle(TeamKeyManager $keys): int
    {
        ...
        if ($failed !== []) {
            ...
            $this->line('Keep the old key in ENVSERVER_PREVIOUS_MASTER_KEYS until this is resolved.');

            return self::FAILURE;
        }

        if ($rewrapped > 0) {
            $this->line('You can now remove the old key from ENVSERVER_PREVIOUS_MASTER_KEYS.');
        }
