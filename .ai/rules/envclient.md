...
## A deploy token needs the scope on both the OAuth token and DeployToken.scopes
`ResolveDeployToken` checks two independent things per middleware.md: the requested scope on the *access token itself* (`$token->cant($scope)`, throws Passport's `MissingScopeException` = "Invalid scope(s) provided.") and `DeployToken::allows($scope)` (the DB column). `accessToken()` in root.go requests `ENVCLIENT_SCOPES` (default `"env:read env:write"`) during the client-credentials exchange — under-requesting here (it used to default to `env:read` only) fails with that exact confusing message even for a token whose `DeployToken.scopes` genuinely includes `env:write`. Requesting a scope the token doesn't actually have is harmless (Passport hands out any scope it knows to any client; `DeployToken::allows()` is the real gate), so the CLI should keep asking for every scope a deploy token could plausibly need rather than narrowing this default.
