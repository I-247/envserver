...
{
    Artisan::call('envserver:cli-client', ['--env-file' => base_path('.env.envclient-device-test')]);

    return Client::sole();
}

beforeEach(function () {
    file_put_contents(base_path('.env.envclient-device-test'), "ENVSERVER_CLI_CLIENT_ID=\n");
});

afterEach(function () {
    @unlink(base_path('.env.envclient-device-test'));
});

it('publishes the CLI client id so the CLI only needs a server URL', function () {
    $client = deviceClient();
    config(['envserver.cli_client_id' => $client->getKey()]);
...
it('says so plainly when no CLI client has been created yet', function () {
    config(['envserver.cli_client_id' => null]);
