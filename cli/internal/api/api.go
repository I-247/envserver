// Package api talks to a Kluis server.
package api
...
// Client is an authenticated connection to one Kluis server.
type Client struct {
	Server string
	Token  string
	HTTP   *http.Client
...
func defaultMessage(status int) string {
	...
	case http.StatusUnauthorized:
		return "not logged in; run \"kluis login\""
	case http.StatusForbidden:
		return "your token is not allowed to do that"
	case http.StatusNotFound:
		return "not found; check the team, project and environment in kluis.json"
	case http.StatusServiceUnavailable:
		return "this server has no CLI client yet; ask an admin to run \"php artisan kluis:cli-client\""
	default:
