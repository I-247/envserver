...
// FileName is the sealed file, written next to envclient.json.
const FileName = ".env.envclient"

const (
	filePrefix    = "envclient-vault"
	formatVersion = "v1"
	...
	keyLabel = "envclient vault key v1"
	idLabel  = "envclient vault key id v1"
)
...
var ErrCorrupt = errors.New("this is not a readable envclient vault file")
...
//
//	envclient-vault.v1.<header>.<nonce>.<ciphertext>
...
func split(contents string) (meta header, encodedMeta string, nonce, ciphertext []byte, err error) {
	...
	if parts[1] != formatVersion {
		return meta, "", nil, nil, fmt.Errorf(
			"this vault is format %s and this envclient only reads %s; upgrade the CLI", parts[1], formatVersion)
	}
