...
func TestFindProjectWalksUpFromASubdirectory(t *testing.T) {
	...
	write(t, filepath.Join(root, ProjectFileName),
		`{"server":"https://kluis.test","team":"acme","project":"shop","environment":"production"}`)
...
func TestFindProjectRejectsAnIncompleteFile(t *testing.T) {
	dir := t.TempDir()
	write(t, filepath.Join(dir, ProjectFileName), `{"server":"https://kluis.test"}`)
...
func TestSaveAndReloadAProject(t *testing.T) {
	...
	project := &Project{Server: "https://kluis.test", Team: "acme", Name: "shop", Environment: "development"}
...
func TestCredentialsRoundTripPerServer(t *testing.T) {
	t.Setenv("KLUIS_CONFIG_DIR", t.TempDir())
...
func TestCredentialsIgnoreATrailingSlash(t *testing.T) {
	...
	if err := SaveCredentials("https://kluis.test/", Credentials{AccessToken: "x"}); err != nil {
		t.Fatal(err)
	}

	if _, found, _ := LoadCredentials("https://kluis.test"); !found {
...
func TestMissingCredentialsAreNotAnError(t *testing.T) {
	...
	_, found, err := LoadCredentials("https://kluis.test")
	if err != nil || found {
