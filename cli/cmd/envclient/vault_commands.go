...
func sealCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Fetches the release once and writes it encrypted to " + vault.FileName + ".\n" +
			...
			"and \"envclient run\" can hand its variables to a command afterwards\n" +
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			p.Note("Open it with: envclient run -- <command>")

			return nil
		},
	}

	cmd.Flags().StringVarP(&file, "file", "f", "", "where to write the sealed file (default: "+vault.FileName+" next to envclient.json)")
...
func unsealCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Prints the variables in .env format on stdout, or writes them to a\n" +
			...
			"Prefer \"envclient run\" where you can: it never puts the values on disk\n" +
...
// ENVCLIENT_VAULT_KEY is the way out for a laptop that logs in interactively and
// therefore has no client secret to derive from.
func vaultSecret() (vault.Secret, error) {
	if key := os.Getenv("ENVCLIENT_VAULT_KEY"); key != "" {
		return vault.FromKey(key)
	}

	if deployTokenSet() {
		return vault.FromDeployToken(os.Getenv("ENVCLIENT_CLIENT_ID"), os.Getenv("ENVCLIENT_CLIENT_SECRET")), nil
	}

	return vault.Secret{}, errors.New(
		"a local vault is locked with your deploy token, and none is set.\n" +
			"Either export ENVCLIENT_CLIENT_ID and ENVCLIENT_CLIENT_SECRET, or pick your own key:\n" +
...
func openVault(override string) (*vault.Payload, string, error) {
	...
	if errors.Is(err, os.ErrNotExist) {
		return nil, path, fmt.Errorf("%s does not exist yet; create it with \"envclient seal\"", path)
	}
	if err != nil {
		return nil, path, err
	}
	...
	if errors.Is(err, vault.ErrWrongKey) {
		...
		return nil, path, fmt.Errorf(
			"%w (%s was sealed with key %s).\n"+
				"Rotated the deploy token? Run \"envclient seal\" again with the new one",
